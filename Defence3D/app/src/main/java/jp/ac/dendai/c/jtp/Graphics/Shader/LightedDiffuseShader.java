...
public class LightedDiffuseShader extends Shader {
    protected int u_fog;
    protected int u_fog_color;
    protected int u_LightedColor;
    protected int u_LightedDirection;
    protected int u_NomalMatrix;
    protected int ma_Normal;
    protected boolean light_direction_update = true;
    ...
    protected float[] light_color = {1,1,1};
    protected float[] light_direction = {0f,0f,1.0f};

    public LightedDiffuseShader(){
        super(FileManager.readTextFile("LightedDiffuseShaderVertex")
                ,FileManager.readTextFile("LightedDiffuseShaderFragment"));
    }
    ...
    void loadShaderVariable() {
        ...
        u_LightedColor = GLES20Util.getUniformLocation(program,"u_LightColor");
        u_LightedDirection = GLES20Util.getUniformLocation(program,"u_LightDirection");
        u_NomalMatrix = GLES20Util.getUniformLocation(program,"u_NormalMatrix");
        ma_Normal = GLES20Util.getAttributeLocation(program,"a_Normal");
    }
    ...
    public void draw(Mesh mesh, float x, float y, float z, float scaleX, float scaleY, float scaleZ, float degreeX, float degreeY, float degreeZ,float alpha,GLES20COMPOSITIONMODE mode) {
        ...
        GLES20.glUniformMatrix4fv(u_NomalMatrix,1,false,normalMatrix,0);

        //法線
        GLES20.glVertexAttribPointer(ma_Position, 3, GLES20.GL_FLOAT, false, GLES20Util.FSIZE * 8, 3);
        GLES20.glEnableVertexAttribArray(ma_Normal);  // バッファオブジェクトの割り当ての有効化
        ...
        //ライト色
        GLES20.glUniform3fv(u_LightedColor,1,light_color,0);
        if(light_direction_update){
            Vector3.normalize(light_direction);
            light_direction_update = false;
        }
        GLES20.glUniform3fv(u_LightedDirection,1,light_direction,0);
