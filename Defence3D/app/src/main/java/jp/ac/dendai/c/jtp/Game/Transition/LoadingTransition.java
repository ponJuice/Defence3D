package jp.ac.dendai.c.jtp.Game.Transition;
...
public class LoadingTransition implements Transitionable {
	...
	private LoadingTransition(){
		nowLoading = new Text("NOW LOADING...");
	...
	public boolean Transition() {
		GameManager.uiShader.updateCamera();
		GameManager.uiShader.useShader();
		if(mode == 0){
		...
		if(mode == 1){
			...
			nowLoading.draw(GameManager.uiShader);
			transitionTime--;
			if(transitionTime <= 0)
				mode = 2;
			return true;
